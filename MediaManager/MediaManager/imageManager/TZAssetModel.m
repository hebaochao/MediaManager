...
- (void)setResult:(id)result {
    ...
    [[TZImageManager sharemanager] getAssetsFromFetchResult:result allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
        _models = models;
        if (_selectedModels) {
...
- (void)checkSelectedModels {
    ...
    for (TZAssetModel *model in _models) {
        if ([[TZImageManager sharemanager] isAssetsArray:selectedAssets containAsset:model.asset]) {
            self.selectedCount ++;
        }
    }
}
