...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(TZAlbumModel *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            if ([collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"Camera Roll"] ||  [collection.localizedTitle isEqualToString:@"All Photos"] || [collection.localizedTitle isEqualToString:@"All Photos"]) {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            if ([name isEqualToString:@"Camera Roll"] || [name isEqualToString:@"Camera Roll"] || [name isEqualToString:@"All Photos"] || [name isEqualToString:@"All Photos"]) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<TZAlbumModel *> *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            ...
            if ([collection.localizedTitle containsString:@"Deleted"] || [collection.localizedTitle isEqualToString:@"Deleted"]) continue;
            if ([collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"All Photos"] || [collection.localizedTitle isEqualToString:@"All Photos"]) {
        ...
        for (PHAssetCollection *collection in albums) {
            PHFetchResult *fetchResult = [PHAsset fetchAssetsInAssetCollection:collection options:option];
            if (fetchResult.count < 1) continue;
            if ([collection.localizedTitle isEqualToString:@"My Photo Stream"] || [collection.localizedTitle isEqualToString:@"My Photo Stream"]) {
                if (albumArr.count) {
        ...
        [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            ...
            } else if ([name isEqualToString:@"My Photo Stream"] || [name isEqualToString:@"My Photo Stream"]) {
                if (albumArr.count) {
...
- (void)getPostImageWithAlbumModel:(TZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        ...
        [[TZImageManager sharemanager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (NSString *)getNewAlbumName:(NSString *)name {
    if (iOS8Later) {
        NSString *newName;
        if ([name rangeOfString:@"Roll"].location != NSNotFound)         newName = @"Camera Roll";
        else if ([name rangeOfString:@"Stream"].location != NSNotFound)  newName = @"My Photo Stream";
        else if ([name rangeOfString:@"Added"].location != NSNotFound)   newName = @"Added";
        else if ([name rangeOfString:@"Selfies"].location != NSNotFound) newName = @"Selfies";
        else if ([name rangeOfString:@"shots"].location != NSNotFound)   newName = @"Shots";
        else if ([name rangeOfString:@"Videos"].location != NSNotFound)  newName = @"Videos";
        else if ([name rangeOfString:@"Panoramas"].location != NSNotFound)  newName = @"All Photos";
        else if ([name rangeOfString:@"Favorites"].location != NSNotFound)  newName = @"Favorites";
        else newName = name;
        return newName;
    } else {
        return name;
